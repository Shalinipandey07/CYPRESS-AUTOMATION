    ...
    function Writefile() {
      ...
      function userID_Alpha() {
        ...
        it('Verify to Add Member', () => {
          ...
          cy.get('input[name=email]').type('shalini03@yopmail.com')
          ...
          cy.get('#OpenProfile').click()
            ...
            cy.get('[name=firstName]').clear().type('Apoorrv'/*"Apoorv"+userID_Alpha()*/)
            cy.get('[name=lastName]').clear().type('Bansal'/*+userID_Alpha()*/)
            cy.get('[name=title]').clear().type('Lead Engineer')
            cy.get('[name=email]').clear().type('apoorv034@yopmail.com'/*email1*/)
        ...
        it('Verify to edit the details of the Member', () => {
          ...
          cy.get('#OpenProfile').click()
            cy.get('[role=menu]').contains('Settings').click()
            cy.get(':nth-child(2) > .text-right > :nth-child(2) > .text-indigo-600').click()
            cy.get('[name=firstName]').clear().type('Parasss')/*("Apoorv"+userID_Alpha())*/
            cy.get('[name=lastName]').clear().type('Madaan')/*("Bansal"+userID_Alpha())*/
            cy.get('[name=title]').clear().type('Lead Engineer')
            cy.get('[type=submit]').should('be.visible').should('be.enabled').click()
            cy.wait(2000)
        ...
        it('Verify to Add Member by clicking on Add member button', () => {
          ...
          cy.get('#OpenProfile').click()
            cy.get('[role=menu]').contains('Settings').click()
            cy.get('button:contains(Add Multiple Users)').click()
            ...
            cy.get('[type=submit]').should('be.visible').should('be.enabled').click()
            cy.wait(2000)*/
            cy.get('.mt-1 > .w-full').click().type('Kajallll, Snehi, kajal1@yopmail.com, QA').type('{enter}').type('Rakeshhh, Suyal, rakesh1@yopmail.com, QA')
